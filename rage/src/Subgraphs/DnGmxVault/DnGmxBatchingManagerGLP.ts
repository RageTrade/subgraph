...
import {
  BatchDeposit,
  DepositToken,
} from './generated/DnGmxBatchingManagerGLP/DnGmxBatchingManagerGLP';
...
// only for the DnGmxJuniorVault, which also allows deposit of GLP
...
export function handleDepositToken(event: DepositToken): void {
  log.debug(
    ...
    [
      ...
      event.params.amount.toString(), // GLP amount
    ]
  );
  ...
  entry.tokenAmount = BigIntToBigDecimal(event.params.amount, token.decimals); // GLP
